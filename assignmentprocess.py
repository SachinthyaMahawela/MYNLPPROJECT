...
class P(QDialog):
    def __init__(self):
        ...
        self.setWindowTitle('Process the assignment')
    ...
    def getText(self):
        ...
        mystring =str(words)
        print("-----------------------------------")
        print('following is the worllist after converting to string')
        print(mystring)
        print(words)
        aftertokenize = word_tokenize(mystring)
        print(aftertokenize)
        #fsplit = symb_remove.split()
        count = len(words)
        ...
        uniq = [word for word in filename.lower().split() if word not in stop_words]
        no_unique = list(set(uniq))
        unique = len(no_unique)
        print('03.The number of unique words of the file:\n', unique)
        print("-----------------------------------")
        ...
        # No of paragraphs
        para = filename.splitlines()
        paragraphs = len(para)
        print("06.Number of paragraphs of given file:\n", paragraphs)
        print("-----------------------------------")

        # frequency of stop words separately.....
        print("07.The frequncy of stop words:")
        ...
        tags = nltk.pos_tag(tokens)  # Tagging the words..into verb, adjectives, nouns etc.
        print("-----------------------------------")
        ...
        cnx = mysql.connector.connect(user='sachi', password='Abc123@#(', host='localhost', database='projectdb')
        cursor = cnx.cursor()
        insertSQL = "INSERT INTO document_data(subject, no_of_words,no_of_stopwords,no_of_uniquewords,no_of_paragraphs,no_of_sentences,no_of_nouns,no_of_verbs,no_of_adjectives,no_of_adverbs)" \
                    "VALUES('" + str(subject) + "','" + str(count) + "','" + str(len(texts)) + "','" + str(len(uniq)) + "','" + str(len(sent)) + "','" + str(len(para)) + "','" + str(len(nouns)) + "','" + str(len(verbs)) + "','" + str(len(adject)) + "','" + str(len(adverb)) + "')"
